...
<section class="relative bg-gradient-to-r from-[rgb(242,240,235)] to-[rgb(64,52,42)]">
  <div class="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center px-6 py-36">
    ...
    <div class="text-[rgb(64,52,42)]">
      ...
      <div class="flex">
        <select class="w-full px-4 py-3 rounded-l-lg border border-gray-300 focus:outline-none">
          <option value="">Pilih desain</option>
          @foreach($allcts as $category)
            <option value="{{ $category->id }}">{{ $category->nama_categori }}</option>
          @endforeach
        </select>
        <a href="{{ route('show.kategory') }}" class="px-5 bg-[rgb(64,52,42)] text-white rounded-r-lg hover:opacity-90">Cari</a>
      </div>
    </div>
    ...
    <div class="flex justify-center relative w-full h-[400px] overflow-hidden rounded-xl shadow-lg">
      <div id="carousel" class="flex transition-transform duration-700 ease-in-out w-full">
        <img src="{{ asset('upload/image.png') }}" alt="Desain 1" class="w-full object-cover rounded-xl">
        <img src="{{ asset('upload/rumah.jpg') }}" alt="Desain 2" class="w-full object-cover rounded-xl">
        <img src="{{ asset('upload/rumah1.jpg') }}" alt="Desain 3" class="w-full object-cover rounded-xl">
      </div>
    </div>
  </div>
</section>
...
<section class="max-w-6xl mx-auto px-6 py-16">
  ...
  <div class="grid md:grid-cols-2 gap-10">
    <div class="space-y-6">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-sm text-gray-600">Dibangun dengan bahan berkualitas tinggi untuk ketahanan maksimal.</p>
        </div>
      </div>
    ...
    <div class="bg-gray-50 rounded-xl p-8 shadow">
      ...
      <select class="w-full px-4 py-3 border rounded-lg focus:outline-none mb-6">
        @foreach($allcts as $category)
          <option value="{{ $category->id }}">{{ $category->nama_categori }}</option>
        @endforeach
      </select>
      <a href="{{ route('show.kategory') }}" class="w-full block text-center px-6 py-3 bg-[rgb(64,52,42)] text-white rounded-lg hover:opacity-90">
        Cari Desain
      </a>
    </div>
  </div>
...
<section class="max-w-7xl mx-auto px-6 py-16">
  <div class="flex justify-between items-center mb-8">
    ...
    <a href="{{ route('show.kategory') }}" class="px-4 py-2 bg-[rgb(64,52,42)] text-white rounded-lg hover:opacity-90">Lihat Semua →</a>
  </div>

  <div class="grid md:grid-cols-3 gap-6">
    @foreach($cts as $category)
      <div class="group relative rounded-xl overflow-hidden shadow-lg">
        <img src="{{ !empty($category->photo) ? url($category->photo) : url('upload/rumah.jpg') }}" class="w-full h-64 object-cover">
        <div class="p-4">
          <h3 class="font-bold text-lg">{{ $category->nama_categori }}</h3>
          <p class="text-sm text-gray-500">Design by Nazarch</p>
        </div>
        <div class="absolute inset-0 bg-black/70 text-white opacity-0 group-hover:opacity-100 transition duration-300 p-6 flex flex-col justify-center">
          <a href="{{ route('kategori.detail', $category->id) }}" class="mt-4 bg-white text-black px-4 py-2 rounded-lg">Lihat Detail</a>
        </div>
      </div>
    @endforeach
  </div>
    ...
    @foreach($reviews as $review)
      <div class="bg-white rounded-xl shadow-lg p-6">
        <div class="flex items-center mb-4">
          <img src="{{ asset('upload/user_images/' . $review->user->photo) }}" class="w-14 h-14 rounded-full mr-4">
          <div>
